 ...
 */
@Entity
@Table(name = "outbound_orders", indexes = {
    @Index(name = "idx_outbound_order_number", columnList = "order_number", unique = true),
    @Index(name = "idx_outbound_warehouse", columnList = "warehouse_id"),
    @Index(name = "idx_outbound_customer", columnList = "customer_id"),
    @Index(name = "idx_outbound_status", columnList = "status"),
    @Index(name = "idx_outbound_type", columnList = "business_type"),
    @Index(name = "idx_outbound_date", columnList = "planned_date")
})
public class OutboundOrder extends BaseEntity {
    ...
    private List<OutboundOrderDetail> details = new ArrayList<>();

    // Constructors
    public OutboundOrder() {
    }

    public OutboundOrder(String orderNumber, Warehouse warehouse, BusinessType businessType, LocalDate plannedDate) {
    ...
    public List<OutboundOrderDetail> getDetails() {
        return details;
    }

    public void setDetails(List<OutboundOrderDetail> details) {
     ...
     * 添加明细
     */
    public void addDetail(OutboundOrderDetail detail) {
        if (details == null) {
     ...
     * 移除明细
     */
    public void removeDetail(OutboundOrderDetail detail) {
        if (details != null) {
            details.remove(detail);
            detail.setOutboundOrder(null);
            recalculateTotal();
        }
    }
    ...
    public boolean canSubmit() {
        return status == ApprovalStatus.DRAFT || status == ApprovalStatus.PENDING;
    }
    ...
    public void execute(User operator) {
        if (!canExecute()) {
            throw new IllegalStateException("当前状态不允许执行出库");
        }
        this.operator = operator;
